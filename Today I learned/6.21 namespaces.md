...
{'name': 'Jin'}
Yes
{'name': 'Jin', 'will_he_be_successful': <function introduce_myself.<locals>.will_he_be_successful at 0x0000029C5DE89AF0>}
```
...
Since `enclosing()` function doesn't exist, we use `print(locals())` to figure out enclosed namespaces.

First time we print `locals()`, we have only `{'name': 'Jin'}`, 

and the second time we print `locals()`, we have `{'name': 'Jin', 'will_he_be_successful': <function introduce_myself.<locals>.will_he_be_successful at 0x0000029C5DE89AF0>}`
